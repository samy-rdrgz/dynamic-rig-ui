...
class RIG_UI_OT_toggle_controllers(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        active_rig = (
            ...
            if bpy.context.active_object.type == "ARMATURE"
            else bpy.context.active_object.parent
        )
        property_bone = active_rig.pose.bones["PROPERTIES"]
        list_collections = "\n" + "\n".join(
            [i.name for i in bpy.data.armatures[active_rig.data.name].collections]
        )
        if param == "all":
            collections = [
                bpy.data.armatures[active_rig.data.name].collections[i.group(0)]
                for i in re.compile(
                    r"^([A-Z]+)_?([A-Z0-9_]+)?(.[LMR])?.?(\d)?$", re.MULTILINE
                ).finditer(list_collections)
            ]
        else:
            collections = [
                bpy.data.armatures[active_rig.data.name].collections[i.group(0)]
                for i in re.compile(
                    rf"^({param})_?([A-Z0-9_]+)?(.[LMR])?.?(\d)?$", re.MULTILINE
                ).finditer(list_collections)
            ]
...
class RIG_UI_PT_rigui(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        p = re.compile(
            r"^([A-Z]+)_?([A-Z0-9_]+)?(.[LMR])?.?(\d)?(PROP)?$", re.MULTILINE
        )
        ...
        collections = [
            ...
            for i in controllers_collections
        ]
        ...
        icon = (
            ...
            if any(
                [
                    i.is_visible
                    for i in bpy.data.armatures[active_rig.data.name].collections
                    if i.name.endswith(".PROP") == False and i.name.isupper()
                ]
            )
            else "HIDE_ON"
        )
        title.operator(
            f"{rig_name.lower()}.toggle_controllers", emboss=False, text="", icon=icon
        ).param = "all"
        icon = (
            "RIGHTARROW"
            if any(
                [
                    active_rig.data[i]
                    for i in list(active_rig.data.keys())
                    if i.startswith("ui_ctrl_")
                ]
            )
            == False
            else "DOWNARROW_HLT"
        ...
        title.operator(
            f"{rig_name.lower()}.toggle_boxes", emboss=False, text="", icon=icon
        ).param = "ui_ctrl_"
        title_txt = title.row()
        ...
        for i, (collection, part, sub_part, side, custom_side, is_prop) in enumerate(
            collections, start=0
        ):
            if box_title != part:
                try:
                    need_box = part == collections[i + 1][1]
                except IndexError:
                    need_box = False
                if need_box:
                ...
                if not collapsible_box:
                    ...
                    title_collapse.prop(
                        ...
                        text="",
                        icon="HIDE_ON"
                        if collection.is_visible == False
                        else "HIDE_OFF",
                        emboss=False,
                    )
                    ...
                    part_collections = [
                        i.group(0)
                        for i in re.compile(
                            rf"^({part})_?([A-Z0-9_]+)?(.[LMR])?.?(\d)?$", re.MULTILINE
                        ).finditer(list_collections)
                    ]
                    toggle = any(
                        [
                            ...
                            for i in bpy.data.armatures[
                                active_rig.data.name
                            ].collections
                            if i.name in part_collections
                        ]
                    )
                    icon = (
                        "RIGHTARROW"
                        if active_rig.data["ui_ctrl_" + part] == False
                        else "DOWNARROW_HLT"
                    )
                    title_eye.prop(
                        active_rig.data,
            ...
            if collapsible_box and active_rig.data["ui_ctrl_" + part]:
                if is_prop == "PROP":
                    ...
                    for prop in [
                        j
                        for j in property_bone.keys()
                        if j.startswith(f"{part}_{sub_part}")
                    ]:
                        box_line.prop(
                            property_bone,
                            '["' + prop + '"]',
                ...
                if is_prop != "PROP" and side == None:
                    ...
                    if side == ".L":
                        ...
                        if (
                            collection.name[:-1] + "R"
                            in bpy.data.armatures[active_rig.data.name].collections
                        ):
                            box_line.prop(
                                bpy.data.armatures[active_rig.data.name].collections[
                                    collection.name[:-1] + "R"
                                ],
                                "is_visible",
                                toggle=True,
                        ...
                        if (
                            collection.name[:-1] + "L"
                            not in bpy.data.armatures[active_rig.data.name].collections
                        ):
                            box_line = box.row(align=True)
                            box_line.label()
                            name = sub_part if sub_part != None else "MAIN"
                            box_line.prop(
                                collection, "is_visible", toggle=True, text=name
                            )
...
class RIG_UI_PT_customprops(bpy.types.Panel):
    ...
    def poll(self, context):
        ...
        try:
            return (
                active_rig.data.get("rig_id") == rig_id
                and bpy.context.object.mode == "POSE"
            )
        except (AttributeError, KeyError, TypeError):
            return False
    ...
    def draw(self, context):
        ...
        list_props = "\n" + "\n".join(custom_props)
        p = re.compile(r"^([A-Z]+)_([A-Z0-9_]+)(.([LMR]|(\d)))?$", re.MULTILINE)
        props_collections = p.finditer(list_props)
        props = [
            (i.group(0), i.group(1), i.group(2), i.group(4)) for i in props_collections
        ]
        ...
        for i, (prop_name, part, sub_part, side) in enumerate(props, 0):
            ...
            if box_title != part:
                ...
                icon = (
                    "RIGHTARROW"
                    if active_rig.data["ui_prop_" + part] == False
                    else "DOWNARROW_HLT"
                )
                titre.prop(
                    active_rig.data,
            ...
            if active_rig.data["ui_prop_" + part]:
                if side is None:
                    ...
                    num_col = 0
                    if prop_name.replace(".L", ".R") not in props_names:
                        empty_space = 1
                elif side == "R":
                    if prop_name.replace(".R", ".L") not in props_names:
                        empty_space = -1
                        num_col = 0
                    else:
                        num_col = 1
...
class RIG_UI_OT_toggle_masks(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if name != "all":
            ...
            masksModifiers = [
                i for i in masksModifiers if i.vertex_group.startswith("MASK_")
            ]
...
class RIG_UI_PT_masks(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        meshes_objs = [i.name for i in active_rig.children]
        modifiers = sum([list(bpy.data.objects[i].modifiers) for i in meshes_objs], [])
        masksModifiers = sorted(
            [i for i in modifiers if i.type == "MASK"],
            key=lambda name: name.vertex_group,
        )
        ...
        p = re.compile(r"^(MASK_)([A-Z0-9_]+)(.([LMR]|(\d)))?$", re.MULTILINE)
        masks = [
            (
                i,
                list(p.finditer(i.vertex_group))[0].group(0),
                list(p.finditer(i.vertex_group))[0].group(2),
                list(p.finditer(i.vertex_group))[0].group(4),
            )
            for i in masksModifiers
        ]
        ...
        toggle = all(
            [i[0].show_viewport for i in masks if i[0].vertex_group.startswith("MASK_")]
        )
        row.operator(
            f"{rig_name.lower()}.toggle_masks",
            emboss=False,
            text="",
            icon="HIDE_ON" if toggle else "HIDE_OFF",
        ).param = "all"
        row.separator(factor=1.14)
        ...
        for i, (modifier, vg, part, side) in enumerate(masks, 0):
            ...
            if mask_nb[vg] > 0:
                ...
                if side is None:
                    ...
                    num_col = 0
                    if vg.replace(".L", ".R") not in masksModifiers_names:
                        empty_space = 1

                elif side == "R":
                    if vg.replace(".R", ".L") not in masksModifiers_names:
                        empty_space = -1
                        num_col = 0
                    else:
                        num_col = 1
...
class RIG_UI_OT_snap_opposite_kinematic(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if active_bone in leg_ik.values() or active_bone in leg_fk.values():
            ...
            kinematic_dir = (
                "ik snap on fk" if active_bone in leg_ik.values() else "fk snap on ik"
            )

        elif active_bone in arm_ik.values() or active_bone in arm_fk.values():
            fk, ik, limb, prop = (arm_fk, arm_ik, limb, prop)
            kinematic_dir = (
                "ik snap on fk" if active_bone in arm_ik.values() else "fk snap on ik"
            )
            ...
            bpy.ops.wm.text_popup(
                ...
                icon="ERROR",
                title="ERROR",
                message="Select a controller of a switchable kinematic chain.",
            )
            return {"FINISHED"}
        ...
        bpy.ops.wm.text_popup(
            ...
            icon="CHECKMARK",
            title="SWITCH DONE",
            message=f"{side.replace('L', 'LEFT').replace('R', 'RIGHT')} {limb}'s {kinematic_dir}",
        )
        return {"FINISHED"}
...
class RIG_UI_OT_reload_ui(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        items = {
            "RIG_UI_PT_rigui": RIG_UI_PT_rigui,
...
class RIG_UI_PT_settings(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.box().row()
        row.operator(
            f"{rig_name.lower()}.reload_ui", emboss=True, text="", icon="FILE_REFRESH"
        )
        col = layout.column(align=True)
...
def init_ui_properties():
    ...
    ctrl_list = set(
        [i.name.split("_")[0].split(".")[0] for i in collections if i.name.isupper()]
    )
    prop_list = set([i.split("_")[0] for i in property_bone.keys()])

    for i in ctrl_list:
        if str("ui_ctrl_" + i) not in rig.data.keys():
            rig.data["ui_ctrl_" + i] = True
    for i in prop_list:
        if str("ui_prop_" + i) not in rig.data.keys():
